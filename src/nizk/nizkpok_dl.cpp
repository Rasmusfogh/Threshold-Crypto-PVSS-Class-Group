...
NizkPoK_DL::NizkPoK_DL(HashAlgo& hash, RandGen &rand, const CL_HSMqk &cl, const SecLevel& seclevel) 
    :   l_boundary(static_cast<unsigned long>(cl_.lambda_distance())),
        Nizk_base(hash, rand, cl),
        l_(cl_.lambda_distance()),
        u_(l_)
{
    b_.reserve(l_);
    ...
    Mpz S_;
    Mpz::mul(A_, cl.encrypt_randomness_bound(), rand.random_mpz_2exp(seclevel.soundness()));
